...
export const translations: Record<string, TranslationSet> = {
	en: {
		...
		footer: {
			...
			aboutContent: [
				...
				"That's why I developed gif2webp. This service uses WASM technology to convert GIF to WebP without sending images to the server. Really! If you don't trust the deployed code, this service is open source, so you can access the source code through the link at the bottom and even host it yourself.",
			],
		},
		errors: {
	...
	ko: {
		...
		footer: {
			about: "소개",
			sourceCode: "소스 코드",
			aboutTitle: "gif2web.com 소개",
			aboutContent: [
				"gif2webp은 브라우저에서 GIF를 WebP로 변환하는 서비스입니다. 이 서비스를 만들게 된 계기는 어떤 디자인 포트폴리오 호스팅 서비스가 GIF 지원을 중단하고 WebP만 지원하기로 결정했을 때, 적절한 변환 도구를 찾을 수 없었기 때문입니다. 특히, 브라우저에서 직접 작동하는 구현을 찾을 수 없었습니다.",
				...
				"그래서 gif2webp을 개발했습니다. 이 서비스는 WASM 기술을 사용하여 이미지를 서버로 전송하지 않고 GIF를 WebP로 변환합니다. 정말입니다! 배포된 코드를 신뢰할 수 없다면, 이 서비스는 오픈 소스이므로 하단의 링크를 통해 소스 코드에 접근할 수 있으며 직접 호스팅할 수도 있습니다.",
			],
		},
		errors: {
